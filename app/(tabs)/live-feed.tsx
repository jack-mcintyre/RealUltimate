...
export default function LiveFeedScreen() {
    ...
    const handleJoinWatchParty = async () => {
        ...
        setIsLoading(true);
        try {
            const teamId = await TeamService.joinTeamByCode(accessCode.toUpperCase());
            if (teamId) {
                // Fetch team details for names
                TeamService.subscribeToTeam(teamId, (t) => setTeam(t));

                // Subscribe to the game
                LiveFeedService.subscribeToActiveGame(teamId, (game) => {
                    setActiveGame(game);
                });
            } else {
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.centerContainer}>
            <Ionicons name="radio-outline" size={80} color="#ff4444" style={{ marginBottom: 20 }} />
            <Text style={styles.welcomeTitle}>Spectator Zone</Text>
            <Text style={styles.subtitle}>Enter your team's access code to watch live play-by-play.</Text>
            ...
            <View style={styles.formContainer}>
                ...
                />
                <TouchableOpacity style={styles.primaryButton} onPress={handleJoinWatchParty} disabled={isLoading}>
                    <Text style={styles.primaryButtonText}>{isLoading ? 'Connecting...' : 'Join Live Feed'}</Text>
                </TouchableOpacity>
            </View>
