...
export default function RecorderScreen() {
    const [teamMode, setTeamMode] = useState<'none' | 'active'>('none');

    // Our Team State
    const [ourAccessCode, setOurAccessCode] = useState('');
    ...
    const { gameState, recordEvent, undo, canUndo, startGame } = useGame(ourTeam ? 'active-game' : undefined);

    const [isLoading, setIsLoading] = useState(false);

    // 1. Identify Our Team
    const handleLoadOurTeam = async () => {
        if (!ourAccessCode) return;
        setIsLoading(true);
        try {
            const teamId = await TeamService.joinTeamByCode(ourAccessCode.toUpperCase());
            if (teamId) {
                TeamService.subscribeToTeam(teamId, (t) => {
                    setOurTeam(t);
                    setTeamMode('active');
                });
            } else {
                Alert.alert("Error", "Invalid Access Code for your team.");
            }
        } catch (e) {
            console.error(e);
    ...
    const handleStartGame = async () => {
        if (!ourTeam || !opponentAccessCode) return;
        setIsLoading(true);
        try {
            // Find Opponent Team ID from their code
            const oppTeamId = await TeamService.joinTeamByCode(opponentAccessCode.toUpperCase());
            if (!oppTeamId) {
            ...
            // Optional: Fetch Opponent details to show their name (Not Strictly required for MVP but good)
            TeamService.subscribeToTeam(oppTeamId, (t) => setOpponentTeam(t));

            // Start the actual game!
            await startGame(ourTeam.id, oppTeamId);
    ...
    // RENDER: Step 1 - Load Our Team
    if (teamMode === 'none') {
        return (
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.centerContainer}>
                <Ionicons name="clipboard-outline" size={80} color="#007AFF" style={{ marginBottom: 20 }} />
                <Text style={styles.welcomeTitle}>Coach Recorder</Text>
                <Text style={styles.subtitle}>Enter your team's access code to manage games.</Text>
                ...
                <View style={styles.formContainer}>
                    <TextInput
                        ...
                        maxLength={6}
                        value={ourAccessCode}
                        onChangeText={setOurAccessCode}
                        autoCapitalize="characters"
                    />
                    <TouchableOpacity style={styles.primaryButton} onPress={handleLoadOurTeam} disabled={isLoading}>
                        <Text style={styles.primaryButtonText}>{isLoading ? 'Loading...' : 'Load Team'}</Text>
                    </TouchableOpacity>
                </View>
            ...
            {!gameState.isGameActive ? (
                <View style={styles.card}>
                    ...
                    <TextInput
                        ...
                        autoCapitalize="characters"
                    />
                    <TouchableOpacity style={[styles.primaryButton, { backgroundColor: '#34C759' }]} onPress={handleStartGame} disabled={isLoading}>
                ...
                <View style={styles.card}>
                    <View style={styles.scoreboard}>
                        ...
                        <View style={styles.scoreBox}>
                            <Text style={styles.scoreLabel}>{opponentTeam ? 'THEM' : 'OPPONENT'}</Text>
